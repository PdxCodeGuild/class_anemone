...
def check(tickets):          # function to see if the value in the list is in the value of other list IE: [0,1,2,3,4,5] 
    ...
    for i in range(1):
        if player[0] == pick6[0]:
            ticket.append(player[0])          #it will keep checking to see if the values are in the list then append it to itself
        if player[1] == pick6[1]:
            ticket.append(player[1])
        if player[2] == pick6[2]:
            ticket.append(player[2])
        if player[3] == pick6[3]:
            ticket.append(player[3])
        if player[4] == pick6[4]:
            ticket.append(player[4])
        if player[5] == pick6[5]:
            ticket.append(player[5])
        return ticket
